...
- **Unusable capture refuses to guess.** Unsupported devices, interrupted capture, low sample counts, and poor tracking cannot produce a measured “no signals detected” result. Reduced Motion uses a separate fixation-and-jump-target protocol and never pools its baseline with the full protocol.
