...
struct UITestLaunchConfiguration {
  ...
  func prepare() {
    ...
    guard fixture != .onboarding else {
      // `removePersistentDomain` is unreliable for the app's own domain: the
      // values stay readable through the in-memory `UserDefaults.standard`, so
      // a device that has completed onboarding once launches this fixture
      // straight onto Home and the test fails on copy it never had a chance to
      // see. Write the value the fixture actually means.
      defaults.set(false, forKey: "sober.onboarding.complete")
      return
    }
