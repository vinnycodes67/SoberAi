...
final class AppModel: ObservableObject {
  ...
  var baselineReady: Bool {
    ...
    // An unreadable archive means there is no range to compare against, even
    // though the count survives. Reporting ready here would let a check run and
  ...
  func reloadResearchData() async {
    ...
    do {
      ...
      // Zero is the honest active count: an unreadable archive is quarantined
      // rather than left in place, so those sessions are no longer in the
      // working set. `localDataError` is what stops a bare zero reading as
      // deletion -- every surface showing the count must show that too.
      clearBaselineState()
      researchDataError = error.localizedDescription
  ...
  func resetPrototype() {
    ...
    baselineStore.beginDeletion()
    clearBaselineState()
    // Unlike a failed read, a reset really is a deletion, so emptying here is
